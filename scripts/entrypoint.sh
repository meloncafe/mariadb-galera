...
generate_server_config() {
    ...
    local node_address
    node_address="${GALERA_NODE_ADDRESS:-$(get_local_ip)}"

    cat > "${MARIADB_CONF_DIR}/my.cnf" << MYCNF
        ...
        for f in /docker-entrypoint-initdb.d/*; do
            ...
            case "$f" in
                ...
                *.sql)
                    ...
                    ;;
                *.sql.gz)
                    log_info "Running compressed SQL file: $f"
                    gunzip -c "$f" | run_sql_stdin
                    ;;
                *.sql.xz)
                    log_info "Running compressed SQL file: $f"
                    xzcat "$f" | run_sql_stdin
                    ;;
                *.sql.zst)
                    log_info "Running compressed SQL file: $f"
                    zstd -dc "$f" | run_sql_stdin
                    ;;
                *)
                    log_warn "Ignoring unknown file type: $f"
                    ;;
            esac
        done
    fi
